          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','user_name') || $check_field('add','user_name') || $check_field('set','user_name')" label="用户姓名" name="user_name">
                                <uni-easyinput type="text" v-model="form['user_name']" v-if="user_group === '管理员' || (form['individual_users_id'] && $check_field('set','user_name')) || (!form['individual_users_id'] && $check_field('add','user_name'))" :disabled="disabledObj['user_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','user_gender') || $check_field('add','user_gender') || $check_field('set','user_gender')" label="用户性别" name="user_gender">
                  ...
                  v-if="user_group === '管理员' || (form['individual_users_id'] && $check_field('set','user_gender')) || (!form['individual_users_id'] && $check_field('add','user_gender'))"
                ></uni-data-select>
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','user_age') || $check_field('add','user_age') || $check_field('set','user_age')" label="用户年龄" name="user_age">
                                <uni-easyinput type="text" v-model="form['user_age']" v-if="user_group === '管理员' || (form['individual_users_id'] && $check_field('set','user_age')) || (!form['individual_users_id'] && $check_field('add','user_age'))" :disabled="disabledObj['user_age_isDisabled']" />
                <!-- 仅查看 -->
...
export default {
  ...
  data() {
    return {
      ...
      group_table: "individual_users",
      is_password: true,
      field: "individual_users_id",
      url_add: "~/api/individual_users/add?",
      url_set: "~/api/individual_users/set?",
      url_get_obj: "~/api/individual_users/get_obj?",
      url_upload: "~/api/individual_users/upload?",

      query: {
        "individual_users_id": 0,
      },
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/individual_users/upload?'),
    ...
    submit_sub(form_sub){
      if(form_sub["individual_users_id"]){
        // 提交事件
        this.$post("~/api/" + this.group_table + "/set?individual_users_id=" + form_sub["individual_users_id"],form_sub,(res)=>{
    ...
    is_view(){
      ...
      if(!bl){
        bl = this.$check_action('/individual_users/table','add');
        console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
      }
      if(!bl){
        bl = this.$check_action('/individual_users/table','set');
        console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
      }
      if(!bl){
        bl = this.$check_action('/individual_users/view','add');
        console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
      }
      if(!bl){
        bl = this.$check_action('/individual_users/view','set');
        console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
      }
      if(!bl){
        bl = this.$check_action('/individual_users/view','get');
